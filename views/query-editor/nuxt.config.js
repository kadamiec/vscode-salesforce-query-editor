...
const packageFile = fs.readFileSync(path.resolve(__dirname, 'package.json'), {
  encoding: 'utf-8',
})
...
export default {
  ...
  css: ['~/assets/css/global.css', '~/assets/css/vscode-dark.css'],
  ...
  build: {
    ...
    extend(config, ctx) {
      ...
      if (process.env.IS_VSCODE) {
        const nuxtFontLoader = config.module.rules.find(
          (rule) =>
            String(rule.test) == String(/\.(woff2?|eot|ttf|otf)(\?.*)?$/i)
        )
        nuxtFontLoader.use = 'base64-inline-loader?limit=1000&name=[name].[ext]'

        const nuxtImageLoader = config.module.rules.find(
          (rule) =>
            String(rule.test) == String(/\.(png|jpe?g|gif|svg|webp|avif)$/i)
        )
